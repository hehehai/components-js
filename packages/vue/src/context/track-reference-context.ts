 ...
 * @public
 */
export const trackRefContextKey: InjectionKey<TrackReferenceOrPlaceholder> = Symbol();
...
export function useTrackRefContext() {
  const trackReference = inject(trackRefContextKey);
  if (!trackReference) {
...
export function useMaybeTrackRefContext() {
  return inject(trackRefContextKey);
}
